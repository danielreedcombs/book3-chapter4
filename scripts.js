...
//   import {githubData}from "./scripts"
...
//   if(payload.commits){
//     newCommitsArray.push(payload.commits) 
//   }
...
for(let i=0; i<githubData.length; i++){
  if(githubData[i].actor.login === "stevebrownlee"){
     steveEvent.push(githubData[i].repo.id)
     for( let i = 0; i <steveEvent.length; i++){
      if (steveEvent[i] === steveEvent[0]){
          steveEvent.push(steveRepo);
      }
     }  
}
eventCounter += 1
}
console.log(steveEvent)
...
// EVENT WITH MOST COMMITS! ID: '8026264881
console.log(githubData[12])
...
for( i=0; i < githubData.length; i++){
  if(githubData[i].payload.pull_request){ 
    ...
    langList.push(githubData[i].payload.pull_request.head.repo.language)
    } 
  }
...
for( i=0; i < langList.length; i++){
    if(!langList.includes(langList[i])) {
      langAffected.push(langList[i])
  }
  // Access each key adds one to the value of that key 
 languageAffected[langList[i]] += 1
}
console.log(languageAffected)